...
const LaundryScheduler = ({ user }) => {
  ...
  const createBooking = async (machineType, slot, day) => {
    ...
    try {
      const response = await fetch('http://18.216.15.211:3002/api/bookings', {
        method: 'POST',
        headers: {
  ...
  const removeBooking = async (bookingId, bookingUserEmail) => {
    ...
    try {
      ...
      const response = await fetch(`http://18.216.15.211:3002/api/bookings/${bookingId}`, {
        method: 'DELETE',
      });
      ...
      if (relatedBookingId) {
        const relatedResponse = await fetch(`http://18.216.15.211:3002/api/bookings/${relatedBookingId}`, {
          method: 'DELETE',
        });

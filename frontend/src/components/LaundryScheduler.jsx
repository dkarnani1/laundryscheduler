...
const LaundryScheduler = ({ user }) => {
  ...
  const createBooking = async (machineType, slot, day) => {
    ...
    try {
      const response = await fetch('https://laundryscheduler.com:3002/api/bookings', {
        method: 'POST',
        headers: {
  ...
  const removeBooking = async (bookingId, bookingUserEmail) => {
    ...
    try {
      ...
      const response = await fetch(`https://laundryscheduler.com:3002/api/bookings/${bookingId}`, {
        method: 'DELETE',
      });
      ...
      if (relatedBookingId) {
        const relatedResponse = await fetch(`https://laundryscheduler.com:3002/api/bookings/${relatedBookingId}`, {
          method: 'DELETE',
        });
